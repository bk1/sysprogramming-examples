...
void *run(void *arg) {
  int retcode;
  key_t *sem_key_ptr = (key_t *)arg;
  key_t sem_key = *sem_key_ptr;
...
int main(int argc, char *argv[]) {
  ...
  pthread_t child;
  retcode = pthread_create(&child, NULL, run, &sem_key);
  ...
  handle_error(retcode, "parent: error while joining", PROCESS_EXIT);
  cleanup();
  printf("done\n");
  exit(0);
}
