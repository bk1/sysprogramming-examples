...
int main(int argc, char **argv) {
  ...
  buffer_t *buffers = (buffer_t *) malloc(nfds * sizeof(buffer_t));
  handle_ptr_error(buffers, "malloc", PROCESS_EXIT);
  ...
  for (int i = 0; i < nfds; i++) {
    ...
    fds[i] = fd;
    buffers[i].data = (char *) malloc((buffer_size + 1) * sizeof(char));
    handle_ptr_error(buffers[i].data, "malloc", PROCESS_EXIT);
  }
  int done = FALSE;
  int idx = -1;
  int pos = -1;
  while (! done) {
    for (int i = 0; i < nfds; i++) {
      int n = read(fds[i], buffers[i].data, buffer_size);
      if (n > 0) {
        done = TRUE;
        idx = i;
        pos = n;
  ...
  buffers[idx].data[pos] = '\000';
  printf("for pipe %d name=%s (fd=%d) found data:\n\"%s\"\n", idx, argv[idx + 1], fds[idx], buffers[idx].data);
  sleep(1);
  printf("exiting\n");
  exit(0);
}
